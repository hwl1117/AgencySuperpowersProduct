...
## Obsidian Brain (每日知识学习系统)

Claude 自动从多个知识源学习，写入 Obsidian vault。位于 `obsidian-brain/` 目录。

### 运行方式
```bash
# 手动触发：在 Claude Code 中说 "运行每日学习" 或 "执行 daily-learning"
# 定时运行：已配置 cron（每天 9:03 AM），仅当前会话有效
```

### 配置文件
- `obsidian-brain/config/topics.json` — 主题列表，添加/修改学习主题
- `obsidian-brain/config/feeds.json` — RSS 订阅源，添加/修改博客订阅

### 输出位置
所有笔记写入 Obsidian vault 的 `Claude学习笔记/` 文件夹：
- 主题笔记: `Claude学习笔记/YYYY-MM-DD 主题名.md`
- RSS 摘要: `Claude学习笔记/RSS/YYYY-MM-DD 订阅源名.md`

### MCP 连接
已配置 `mcp-obsidian` MCP server，Claude Code 可直接读写 Obsidian vault。
